...
export function TextField(options = {}) {
    ...
    if (options.placeholder)
        inputEl.setAttribute('aria-label', options.placeholder);
    if (options.disabled)
    ...
    if (options.onFocus) {
        inputEl.addEventListener('focus', options.onFocus);
    }
    if (options.onBlur) {
        inputEl.addEventListener('blur', options.onBlur);
    }
    ...
    if (options.fullWidth)
        wrapperClass += ' wj-textfield--full-width';
    if (hasError)
        wrapperClass += ' wj-textfield--error';
    const wrapper = html `<div class="${wrapperClass}"></div>`;
    const inner = html `<div class="wj-textfield-inner"></div>`;
    inner.appendChild(inputEl);
    if (options.label) {
        const labelEl = html `<label class="wj-textfield-label">${options.label}</label>`;
        inner.appendChild(labelEl);
    }
    wrapper.appendChild(inner);
    const helperEl = html `<div class="wj-textfield-helper">${helperMsg}</div>`;
    wrapper.appendChild(helperEl);
    const extended = wrapper.extend(() => ({
        getValue() {
