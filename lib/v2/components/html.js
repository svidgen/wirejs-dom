...
export function html(
	...
	// try {
	// 	node.setAttribute('wirejs-args', JSON.stringify(builders));
	// } catch {
	// 	console.warn('Some args could not be serialized', node, builders);
	// }
	...
	for (const builder of adjustedBuilders) {
		...
		if (builder?.id) {
			...
			node.setAttribute?.('wirejs-data-id', builder.id);
		}
	}
	...
	if (!nodeAccessor[propName]) {
		...
		Object.defineProperty(
			...
			propName,
			{
				...
				set(v) {
					// set in attributes first for serialization (ssr/ssg)
					try {
						node.setAttribute(`wirejs-data-${propName}`, JSON.stringify(v));
					} catch {
						console.warn("Cannot serialize: ", v);
					}
