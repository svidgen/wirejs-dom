...
export function list(id, mapperOrDataA, mapperOrDataB) {
    ...
    return {
        ...
        bless: (context) => {
            ...
            //
            // initially was using a range to avoid creating "sentinal" nodes.
            // but, when moving a component around, if the range happens to be
            // empty, we need an additional sentinal node to anchor the range
            // anyway ... so, might be worthwhile to rejigger this all to just
            // use empty text node start/end marker instead. push() could then
            // very simply be an insertBefore against the end marker.
            //
            // that said, it ain't "broke" right now. so, not fixing. :shrug:
            // 
