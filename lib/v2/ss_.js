 ...
 * @param {Element & { data: object } | ({data: object}) => Element & { data: object }} replacement - The live element
 * with a `data` property to populate.
 */
export function hydrate(rendered, replacement) {
	...
	if (!renderedNode) {
		hydrate.pending.push({ id: rendered, replacement });
		return;
	}
...
/**
 * @type {{id: string; Element & { data: object } | () => Element & { data: object }})[]}
 */
hydrate.pending = [];