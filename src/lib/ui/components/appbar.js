...
export function AppBar(options = {}) {
    ...
    if (options.elevation !== false)
        classes += ' wj-appbar--elevated';
    const el = html `<header class="${classes}"></header>`;
    if (options.title) {
        const titleEl = html `<div class="wj-appbar-title"></div>`;
    ...
    if (options.children) {
        const contentEl = html `<div class="wj-appbar-content"></div>`;
        appendChildren(contentEl, options.children);
        el.appendChild(contentEl);
    }
    if (options.actions && options.actions.length > 0) {
        const actionsEl = html `<div class="wj-appbar-actions"></div>`;
