...
export function Card(options: CardOptions = {}): HTMLElement {
	...
	let classes = 'wj-card';
	if (options.elevated) classes += ' wj-card--elevated';
	if (options.outlined) classes += ' wj-card--outlined';
	if (options.onClick) classes += ' wj-card--clickable';

	const el = html`<div class="${classes}"></div>` as unknown as HTMLElement;

	if (options.title) {
		const header = html`<div class="wj-card-header"></div>` as unknown as HTMLElement;
		const titleEl = html`<div class="wj-card-title">${options.title}</div>` as unknown as HTMLElement;
		header.appendChild(titleEl);
		if (options.subtitle) {
			const subtitleEl = html`<div class="wj-card-subtitle">${options.subtitle}</div>` as unknown as HTMLElement;
			header.appendChild(subtitleEl);
		}
		el.appendChild(header);
	}

	const content = html`<div class="wj-card-content"></div>` as unknown as HTMLElement;
	appendChildren(content, options.children);
	el.appendChild(content);

	if (options.actions && options.actions.length > 0) {
		const actionsEl = html`<div class="wj-card-actions"></div>` as unknown as HTMLElement;
