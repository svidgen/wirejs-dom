...
export function Dialog(options: DialogOptions = {}): DialogElement {
	...
	const el = html`<dialog class="wj-dialog"></dialog>` as unknown as HTMLDialogElement;

	if (options.title) {
		const titleEl = html`<div class="wj-dialog-title"></div>` as unknown as HTMLElement;
	...
	const contentEl = html`<div class="wj-dialog-content"></div>` as unknown as HTMLElement;
	appendChildren(contentEl, options.children);
	el.appendChild(contentEl);

	if (options.actions && options.actions.length > 0) {
		const actionsEl = html`<div class="wj-dialog-actions"></div>` as unknown as HTMLElement;
	...
	const extended = (el as any).extend(() => ({
		show(): void {
			el.setAttribute('open', '');
		},
		close(): void {
