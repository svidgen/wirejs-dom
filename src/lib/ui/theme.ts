...
function deepMerge<T extends object>(base: T, overrides: DeepPartial<T>): T {
	...
	for (const key of Object.keys(overrides) as (keyof T)[]) {
		...
		if (ov !== undefined && typeof bv === 'object' && bv !== null && typeof ov === 'object') {
			result[key as string] = deepMerge(bv as object, ov as DeepPartial<object>);
...
export function applyTheme(theme: Theme, element?: HTMLElement): void {
	const el: HTMLElement | null = element ?? (typeof document !== 'undefined' ? document.documentElement : null);
	if (!el) return;
